...
////这个是自己添加的方法，跟第三方无关，简单搞，扔这里
- (unsigned long long)getCachedSizeWithURLStr:(NSString *)urlStr;
{
    NSString *cacheFileName = [NSString stringWithFormat:@"FSCache-%@",[urlStr musicHashString]];
    NSArray *contentsArr = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:self.configuration.cacheDirectory error:nil];
    
    for (NSString *file in contentsArr) {
        if ([file isEqualToString:cacheFileName]) {
            NSString *fullPath = [NSString stringWithFormat:@"%@/%@", self.configuration.cacheDirectory, file];
            NSDictionary *attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:fullPath error:nil];
